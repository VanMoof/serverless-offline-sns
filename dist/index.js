...
var ServerlessOfflineSns = /** @class */ (function () {
    ...
    ServerlessOfflineSns.prototype.getFunctionName = function (name) {
        ...
        Object.entries(this.serverless.service.functions).forEach(function (_a) {
            ...
            var events = fp_1.get(["events"], funcValue);
            events &&
                events.forEach(function (event) {
                    var attribute = fp_1.get(["sqs", "arn"], event);
                    if (!fp_1.has("Fn::GetAtt", attribute))
                        return;
    ...
    ServerlessOfflineSns.prototype.getResourceSubscriptions = function (serverless) {
        var _this = this;
        var resources = serverless.service.resources.Resources;
        var subscriptions = [];
        if (!resources)
            return subscriptions;
        new Map(Object.entries(resources)).forEach(function (value, key) {
            var type = fp_1.get(["Type"], value);
            if (type !== "AWS::SNS::Subscription")
                return;
            var endPoint = fp_1.get(["Properties", "Endpoint"], value);
            if (!fp_1.has("Fn::GetAtt", endPoint))
                return;
            var _a = endPoint["Fn::GetAtt"], resourceName = _a[0], attribute = _a[1];
            type = fp_1.get(["Type"], resources[resourceName]);
            if (attribute !== "Arn")
                return;
            if (type !== "AWS::SQS::Queue")
                return;
            var filterPolicy = fp_1.get(["Properties", "FilterPolicy"], value);
            var protocol = fp_1.get(["Properties", "Protocol"], value);
            var rawMessageDelivery = fp_1.get(["Properties", "RawMessageDelivery"], value);
            var topicArn = fp_1.get(["Properties", "TopicArn", "Ref"], value);
            var topicName = fp_1.get(["Properties", "TopicName"], resources[topicArn]);
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                            ...
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        ...
                                        return [4 /*yield*/, sls_config_parser_1.loadServerlessConfig(shell.pwd(), this_1.debug)];
                                    case 1:
                                        serverless = _a.sent();
                                        ...
                                        Object.keys(serverless.service.functions).map(function (fnName) {
                                            ...
                                            subscribePromises.push(Promise.all(fn.events.filter(function (event) { return event.sns != null; }).map(function (event) {
                    ...
                    case 6:
                        ...
                        _b.label = 7;
                    case 7: return [4 /*yield*/, Promise.all(subscribePromises)];
                    case 8:
                        _b.sent();
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (!topicName) {
                            ...
                            return [2 /*return*/, Promise.resolve("Unable to create topic for \"" + queueUrl + "\". Please ensure the sns configuration is correct.")];
                        }
    ...
    ServerlessOfflineSns.prototype.createJavascriptHandler = function (fn, location) {
        ...
        return function () {
            ...
            var fullHandlerPath = path_1.resolve(location, handlerPath);
